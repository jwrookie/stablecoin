...
describe('Dao Locker Supplement', function () {
    ...
    it('test boost so big', async function () {
        ...
        expect(await tra.balanceOf(gauge.address)).to.be.eq(0);
    ...
    it('test boost vote', async function () {
        ...
        await locker.connect(third).createLock(toWei("0.3"), ONE_DAT_DURATION);
        ...
        expectRevert(await gauge.connect(third).getReward(third.address), "TransferHelper: TRANSFER_FROM_FAILED");
        expect(await boost.mintDuration()).to.be.eq(7 * 28800);
        await boost.setMitDuration(360 * 28800);
        await boost.updateAll();
        await time.advanceBlockTo(parseInt(await time.latestBlock()) + 30000 * 8);
